...
start <- function(bot, updates) {
  ...
  tg_username <- data.frame(paste0('=HYPERLINK("https://t.me/',
  ...
  tg_username$paste0...HYPERLINK...https...t.me....updates..length.updates....message.from.username.. <- gs4_formula(tg_username$paste0...HYPERLINK...https...t.me....updates..length.updates....message.from.username..)
  
  data <- data.frame(Sys.Date(), tg_username,updates[[length(updates)]]$message$chat_id)
  
  who <- get_chat_data(updates[[length(updates)]]$message$chat_id, 'chat_id')
  
  if (length(who) == 0) {
  
    bot$sendPhoto(updates[[length(updates)]]$message$chat_id,
                  photo = "rh.jpg",
                  caption = paste0("Я - бот, который помогает записаться на тренировку, продлить абонемент или оплатить хранение.
                             \nДавайте знакомиться, как Вас зовут?")
    )
    
    
    set_state(chat_id = updates[[length(updates)]]$message$chat_id, state = 'start')
    set_chat_data(updates[[length(updates)]]$message$chat_id, 'tg_username', updates[[length(updates)]]$message$from$username)
    ...
    set_state(chat_id = updates[[length(updates)]]$message$chat_id, state = 'wait_name')
      ...
      uname <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'name'))
      ...
      set_state(chat_id = updates[[length(updates)]]$from_chat_id(), state = paste0("wait_",updates[[length(updates)]]$callback_query$data))
...
tren <- function(bot, updates) {
  ...
  RKM <- ReplyKeyboardMarkup(
    keyboard = list(
      list(KeyboardButton("Групповая тренировка")),
      list(KeyboardButton("Индивидуальная тренировка"))
    ),
...
vid_tren <- function(bot, updates) {
  ...
  bot$sendChatAction(
    chat_id = updates[[length(updates)]]$message$chat_id,
    action = "typing"
  )
  ...
  keys <- lapply(1:nrow(date_tren), function(x) list(KeyboardButton(paste0(
    date_tren$date[x],
    " ",
  ...
  bot$sendMessage(chat_id = updates[[length(updates)]]$from_chat_id(),
...
zapis <- function(bot, updates) {
  ...
  date_tren <- updates[[length(updates)]]$message$text %>%
    str_split_fixed(" ", 4)
  date_tren <- paste0(date_tren[,1], " ", date_tren[,2], " ",date_tren[,3])
  ...
  username <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'name'))
  type_teren <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'type_teren'))
  date_tren1 <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'date_tren'))
  treiner1 <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'treiner'))
  # tg_username  <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'tg_username'))
  tg_username <- data.frame(paste0('=HYPERLINK("https://t.me/',
                        data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'tg_username')),
                        '","',
                        data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'tg_username')),
                        '")'
                        ))
  tg_username$paste0...HYPERLINK...https...t.me....data.frame.get_chat_data.updates..length.updates....message.chat_id.. <- gs4_formula(tg_username$paste0...HYPERLINK...https...t.me....data.frame.get_chat_data.updates..length.updates....message.chat_id..)
  chat_id <- data.frame(get_chat_data(updates[[length(updates)]]$message$chat_id, 'chat_id'))
  ...
  range_write(key, data = data.frame(paste0("Бот записал ",username, " на ", type_teren1)), range = paste0("date!F",zapis_data$row_num+1), col_names = FALSE)
  
  Sys.sleep(1)
  
  zapis_data_t <- read_sheet(key, sheet = "events", range = "A:F")
  zapis_data_t$row_num <- seq.int(nrow(zapis_data_t))
  range_write(key, data = username, range = paste0("events!A",(max(zapis_data_t$row_num)+2)), col_names = FALSE)
  range_write(key, data = type_teren, range = paste0("events!B",(max(zapis_data_t$row_num)+2)), col_names = FALSE)
  range_write(key, data = date_tren1, range = paste0("events!C",(max(zapis_data_t$row_num)+2)), col_names = FALSE)
  range_write(key, data = treiner1, range = paste0("events!D",(max(zapis_data_t$row_num)+2)), col_names = FALSE)
  range_write(key, data = tg_username, range = paste0("events!E",(max(zapis_data_t$row_num)+2)), reformat = F, col_names = FALSE)
  range_write(key, data = chat_id, range = paste0("events!F",(max(zapis_data_t$row_num)+2)), col_names = FALSE)
  
  bot$sendMessage(updates[[length(updates)]]$message$chat_id,
                  text = paste0("Отлично, запись на ", 
                                date_tren1,
                                " подтверждена к ",
                                treiner1,
...
state <- function(bot, updates) {
  ...
  chat_state <- get_state(updates[[length(updates)]]$message$chat_id)
...
chat_data <- function(bot, updates) {
  ...
  cd <- dbGetQuery(con, str_interp("SELECT distinct (tg_username) FROM chat_data 
...
wait_vid_tren_h <- MessageHandler(vid_tren, MessageFilters$wait_vid_tren & !MessageFilters$command)
wait_zapis_h <- MessageHandler(zapis, MessageFilters$wait_zapis & !MessageFilters$command)
